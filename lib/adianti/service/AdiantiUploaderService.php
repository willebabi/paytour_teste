    ...
    function show($param)
    {
        ...
        if (isset($_FILES['fileName'])) {
            ...
            if ($file['error'] === 0 && $file['size'] > 0) {
                ...
                if (!empty($param['tamanho']) && $param['tamanho'] > 0) {
                    if ($bytes / 1048576 > $param['tamanho']) {
                        $response = array();
                        $response['type'] = 'error';
                        $response['msg']  = 'Tamanho do arquivo superior a ' . $param['tamanho'] . 'MB. (' . number_format($bytes / 1048576, 2) . ')';
                        echo json_encode($response);
                        return;
                    }
                }
